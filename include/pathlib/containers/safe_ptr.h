...
public:
  ...
  T* operator->() const
  {
    if (EXPECT(this->_ptr != nullptr)) {
      ...
      get_errors().fatal(u8"Attempt to access an object through a null SafePtr.");
      return &_ptr[0];
    }
  }
  ...
  T& operator*() const 
  {
    if (EXPECT(_offset_ptr != nullptr)) {
      ...
      get_errors().fatal(u8"Attempt to access a null SafePtr.");
      return _ptr[0];
    }
  }
  ...
  T& operator[](u64 index) const
  {
    if (EXPECT(((_offset_ptr - _ptr + index) < _count))) {
      ...
      if (_ptr == nullptr) {
        ...
        get_errors().fatal(u8"Out of bounds access to SafePtr.");
      }
      return _ptr[0];
    }
  }
  ...
  const SafePtr operator-(u64 offset) const
  {
    if (EXPECT(((_offset_ptr - offset) < _offset_ptr) &&
      ...
      get_errors().fatal(u8"Out of bounds pointer arithmetic; pointer must remain at, or after, original address.");
  ...
  const SafePtr operator+(u64 offset) const
  {
    if (EXPECT(((_offset_ptr + offset) > _offset_ptr) &&
      ...
      get_errors().fatal(u8"Out of bounds pointer arithmetic.");
      return SafePtr(nullptr, 0);
    }
  }
