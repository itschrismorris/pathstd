...
struct Atomic
{
  ...
  inline void store(const T value,
                    i32 mem_order = SEQ_CST)
  {
    __atomic_store_n(&_value, value, mem_order);
  }

  //---
  inline T load(i32 mem_order = SEQ_CST) const
  {
    return __atomic_load_n(&_value, mem_order);
  }

  //---
  inline T exchange(const T value,
                    i32 mem_order = SEQ_CST) const
  {
    return __atomic_exchange_n(&_value, value, mem_order);
  }

  //---
  inline bool compare_exchange(const T expected_value,
...
struct AtomicFlag : Atomic<u8>
{
  ...
  //---
  inline bool test(i32 mem_order = Atomic<>::SEQ_CST)
  {
    return __atomic_load_n(&_value, mem_order);
  }

  //---
  inline bool test_and_set(i32 mem_order = Atomic<>::SEQ_CST)
  {
    return __atomic_test_and_set(&_value, mem_order);
  }

  //---
  inline void clear(i32 mem_order = Atomic<>::SEQ_CST)
  {
