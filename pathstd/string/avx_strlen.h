...
/**/
#define GET_CMP_MASK(STR, ZERO) ((uint32_t)_mm256_movemask_epi8( \
                                             _mm256_cmpeq_epi8( \
                                               _mm256_load_si256((__m256i*)(STR)), ZERO)))
...
static inline size_t avx_strlen(const char* s)
{
  ...
  int64_t ignore_bytes = s - _s;
  if (ignore_bytes < 64) {
    uint64_t check32 = GET_CMP_MASK(_s, zero);
    uint64_t check64 = GET_CMP_MASK(_s + 32, zero);
  ...
  uint64_t check96 = GET_CMP_MASK(_s + 64, zero);
  uint64_t check128 = GET_CMP_MASK(_s + 96, zero);
  uint64_t zero_mask128 = (check96 | (check128 << 32)) & 
                          (UINT64_MAX << std::max(0LLU, ignore_bytes - 64LLU));
  if (zero_mask128) {
  ...
  s = _s + 128;
  while (true) {
    uint64_t check32 = GET_CMP_MASK(s, zero);
    uint64_t check64 = GET_CMP_MASK(s + 32, zero);
    uint64_t zero_mask64 = (check32 | (check64 << 32));
    if (zero_mask64) {
      return (counted + _tzcnt_u64(zero_mask64));
    }
    uint64_t check96 = GET_CMP_MASK(s + 64, zero);
    uint64_t check128 = GET_CMP_MASK(s + 96, zero);
    uint64_t zero_mask128 = (check96 | (check128 << 32));
    if (zero_mask128) {
